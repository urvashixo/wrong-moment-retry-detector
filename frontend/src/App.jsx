        ...
        {bars.map((h,i)=>{
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full transition-all duration-700 ease-out" style={{height:`${height}px`, background:isPeak?'var(--heat-1)':i===flashIdx+1||i===flashIdx-1?'var(--heat-2)':`color-mix(in srgb, var(--heat-4) ${20+(h/45)*30}%, var(--line))`, boxShadow:isPeak&&animated?'0 0 12px var(--heat-1)':'none', transitionDelay:`${i*18}ms`}}/>
            </div>
          );
        })}
      </div>
...
export default function App() {
  ...
  const [tab,setTab]=useState('feed'); // feed | review | diff | ab
    ...
    <div className="min-h-screen" style={{background:'var(--bg)', color:'var(--text-primary)'}}>
      <header className="sticky top-0 z-40 backdrop-blur" style={{background:'rgba(10,10,10,0.9)', borderBottom:'1px solid var(--line)'}}>
        <div className="max-w-[1160px] mx-auto px-6 h-[48px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-[11px]">
            <button onClick={()=>{setTab('feed'); scrollToLive();}} className="px-2 py-1" style={{border:'1px solid var(--line)', background:tab==='feed'?'var(--panel)':'transparent', color: tab==='feed'?'var(--text-primary)':'var(--text-dim)'}}>[ feed ]</button>
            <button onClick={()=>setTab('review')} className="px-2 py-1" style={{border:'1px solid var(--line)', background:tab==='review'?'var(--panel)':'transparent', color: tab==='review'?'var(--text-primary)':'var(--text-dim)'}}>[ needs review ]</button>
            <button onClick={()=>setTab('diff')} className="px-2 py-1" style={{border:'1px solid var(--line)', background:tab==='diff'?'var(--panel)':'transparent', color: tab==='diff'?'var(--text-primary)':'var(--text-dim)'}}>[ diff ]</button>
            <button onClick={()=>setTab('ab')} className="px-2 py-1" style={{border:'1px solid var(--line)', background:tab==='ab'?'var(--panel)':'transparent', color: tab==='ab'?'var(--text-primary)':'var(--text-dim)'}}>[ A/B ]</button>
      ...
      <section className="max-w-[1160px] mx-auto px-6 pt-10 pb-8">
        <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-10 items-start">
          <div>
            ...
            <button onClick={scrollToLive} className="mt-6 text-sm inline-flex items-center gap-2 hover:opacity-80" style={{color:'var(--text-primary)'}}><span>▸</span> see it find a pattern</button>
      ...
      <section ref={liveRef} id="live" className="max-w-[1160px] mx-auto px-6 pt-12">
        <div className="flex items-end justify-between border-b pb-3" style={{borderColor:'var(--line)'}}>
          ...
          <span className="text-[11px]" style={{color:'var(--text-dim)'}}>timestamps UTC, IST displayed · A/B live split</span>
        </div>
        <div className="mt-6"><SimulationControls onUpdate={triggerRefresh} /></div>
      </section>

      {/* A/B live panel always visible above tabs when tab=ab or as part of feed */}
      {tab==='ab' && (
        <section className="max-w-[1160px] mx-auto px-6 mt-6">
          <ABPanel refreshKey={refreshKey} />
        </section>
      )}

      <section className="max-w-[1160px] mx-auto px-6 mt-8">
        {tab==='feed' && (
          <div className="grid lg:grid-cols-[1fr_380px] gap-6 items-start">
            ...
            <div className="lg:sticky lg:top-[60px] space-y-6">
              {profileId ? (
                <ProfilePage customerId={profileId} onClose={()=>setProfileId(null)} onSelectDecision={(id)=>{ /* could scroll to feed */ }} />
              ) : selectedCustomer ? (
                ...
                <div className="panel p-6 text-xs leading-5" style={{color:'var(--text-dim)'}}>
                  Select a decision log entry to see its histogram and <span className="bracket">[ generated ]</span> explanation.<br/><br/>
                  Click any row — it expands inline, no modal. The number is computed; the sentence is explained.
                  <div className="mt-4 pt-4" style={{borderTop:'1px solid var(--line)'}}>
                    Confidence: <span className="heat-1">[ high ]</span> <span className="heat-2">→</span> <span className="heat-4">[ fallback ]</span> · 
                    <button onClick={()=>setProfileId(selectedCustomer||'')} className="ml-2 underline">open profile page →</button>
                  </div>
                  <div className="mt-4 flex gap-2">
                    <button onClick={()=>setSelectedCustomer && setSelectedCustomer(selectedCustomer)} className="text-[11px] px-2 py-1" style={{border:'1px solid var(--line)'}}>open inline histogram</button>
                    <button onClick={()=>profileId || (selectedCustomer && setProfileId(selectedCustomer))} className="text-[11px] px-2 py-1" style={{border:'1px solid var(--heat-1)', color:'var(--heat-1)'}}>open full profile page</button>
                  </div>
                </div>
              )}
              {/* quick profile access */}
              <div className="panel p-4">
                <div className="text-[11px] tracking-widest" style={{color:'var(--text-dim)'}}>PROFILE QUICK OPEN</div>
                <div className="mt-2 flex gap-2">
                  ...
                  <button onClick={()=>{
                    ...
                    if(v) setProfileId(v);
                  }} className="text-xs px-3 py-1" style={{border:'1px solid var(--line)'}}>open</button>
                </div>
                <div className="mt-2 text-[11px]" style={{color:'var(--text-dim)'}}>GET /api/customers/{'{id}'}/profile — full timeline, histogram, decisions, overrides.</div>
              </div>
            </div>
          </div>
        )}
        {tab==='review' && <NeedsReview refreshKey={refreshKey} onOpen={(cid)=>{ setProfileId(cid); setTab('feed'); }} />}
        {tab==='diff' && <DiffView refreshKey={refreshKey} />}
        {tab==='feed' && (
          <div className="mt-6">
            <ABPanel refreshKey={refreshKey} />
          </div>
      ...
      <footer className="max-w-[1160px] mx-auto px-6 mt-12 py-8" style={{borderTop:'1px solid var(--line)', color:'var(--text-dim)'}}>
        <div className="flex flex-col md:flex-row justify-between gap-4 text-xs">
          <div>
            ...
            <div className="mt-1 max-w-[60ch]">Learns each customer’s personal liquidity window and retries there. Deterministic model decides WHEN; Groq only explains. Overrides and A/B live.</div>
          </div>
